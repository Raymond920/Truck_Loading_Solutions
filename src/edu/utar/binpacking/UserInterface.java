...
public class UserInterface {
	...
	public void displayResults(List<Truck> trucks) {
		...
		for(int i = 0; i < trucks.size(); i++) {
			...
			for(int j = 0; j < trucks.get(i).getPackedParcels().size(); j++) {
				System.out.println(trucks.get(i).getPackedParcels().get(j).getName());
				allParcelWeight += trucks.get(i).getPackedParcels().get(j).getWeight();
				totalWeight += trucks.get(i).getPackedParcels().get(j).getWeight();
			}
			System.out.println("=============================================");
